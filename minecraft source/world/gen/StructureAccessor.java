...
public class StructureAccessor {
    ...
    public List<StructureStart> getStructureStarts(ChunkPos pos, Predicate<Structure> predicate) {
        Map<Structure, LongSet> $$2 = this.world.getChunk(pos.x, pos.z, ChunkStatus.STRUCTURE_REFERENCES).getStructureReferences();
        ImmutableList.Builder $$3 = ImmutableList.builder();
        for (Map.Entry<Structure, LongSet> $$4 : $$2.entrySet()) {
            Structure $$5 = $$4.getKey();
            if (!predicate.test($$5)) continue;
            this.acceptStructureStarts($$5, $$4.getValue(), arg_0 -> ((ImmutableList.Builder)$$3).add(arg_0));
        }
        return $$3.build();
    }

    public List<StructureStart> getStructureStarts(ChunkSectionPos sectionPos, Structure structure) {
        LongSet $$2 = this.world.getChunk(sectionPos.getSectionX(), sectionPos.getSectionZ(), ChunkStatus.STRUCTURE_REFERENCES).getStructureReferences(structure);
        ImmutableList.Builder $$3 = ImmutableList.builder();
        this.acceptStructureStarts(structure, $$2, arg_0 -> ((ImmutableList.Builder)$$3).add(arg_0));
        return $$3.build();
    }
    ...
    public void acceptStructureStarts(Structure structure, LongSet structureStartPositions, Consumer<StructureStart> consumer) {
        ...
        while (longIterator.hasNext()) {
            ...
            ChunkSectionPos $$4 = ChunkSectionPos.from(new ChunkPos($$3), this.world.getBottomSectionCoord());
            StructureStart $$5 = this.getStructureStart($$4, structure, this.world.getChunk($$4.getSectionX(), $$4.getSectionZ(), ChunkStatus.STRUCTURE_STARTS));
            if ($$5 == null || !$$5.hasChildren()) continue;
            consumer.accept($$5);
        }
    }
    ...
    public StructureStart getStructureAt(BlockPos pos, Structure structure) {
        for (StructureStart $$2 : this.getStructureStarts(ChunkSectionPos.from(pos), structure)) {
            if (!$$2.getBoundingBox().contains(pos)) continue;
            return $$2;
        }
    ...
    public StructureStart getStructureContaining(BlockPos pos, Predicate<RegistryEntry<Structure>> predicate) {
        RegistryWrapper.Impl $$2 = this.getRegistryManager().getOrThrow(RegistryKeys.STRUCTURE);
        for (StructureStart $$3 : this.getStructureStarts(new ChunkPos(pos), arg_0 -> StructureAccessor.method_41414((Registry)$$2, predicate, arg_0))) {
            if (!this.structureContains(pos, $$3)) continue;
            return $$3;
        }
        return StructureStart.DEFAULT;
    }
    ...
    public StructureStart getStructureContaining(BlockPos pos, Structure structure) {
        for (StructureStart $$2 : this.getStructureStarts(ChunkSectionPos.from(pos), structure)) {
            if (!this.structureContains(pos, $$2)) continue;
            return $$2;
        }
        return StructureStart.DEFAULT;
    }
    ...
    public boolean structureContains(BlockPos pos, StructureStart structureStart) {
        for (StructurePiece $$2 : structureStart.getChildren()) {
            if (!$$2.getBoundingBox().contains(pos)) continue;
            return true;
        }
        return false;
    ...
    public boolean hasStructureReferences(BlockPos pos) {
        ChunkSectionPos $$1 = ChunkSectionPos.from(pos);
        return this.world.getChunk($$1.getSectionX(), $$1.getSectionZ(), ChunkStatus.STRUCTURE_REFERENCES).hasStructureReferences();
    }

    public Map<Structure, LongSet> getStructureReferences(BlockPos pos) {
        ChunkSectionPos $$1 = ChunkSectionPos.from(pos);
        return this.world.getChunk($$1.getSectionX(), $$1.getSectionZ(), ChunkStatus.STRUCTURE_REFERENCES).getStructureReferences();
    }
    ...
    private static /* synthetic */ boolean method_41414(Registry $$0, Predicate $$1, Structure structure) {
        return $$0.getEntry($$0.getRawId(structure)).map($$1::test).orElse(false);
    }
}
