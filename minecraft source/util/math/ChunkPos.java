...
public class ChunkPos {
    ...
    public static final PacketCodec<ByteBuf, ChunkPos> PACKET_CODEC = new PacketCodec<ByteBuf, ChunkPos>(){
        ...
        public ChunkPos decode(ByteBuf buf) {
            return PacketByteBuf.readChunkPos(buf);
        }

        @Override
        public void encode(ByteBuf buf, ChunkPos value) {
            PacketByteBuf.writeChunkPos(buf, value);
        }
    };
    ...
    public static int hashCode(int x, int z) {
        int $$2 = 1664525 * x + 1013904223;
        int $$3 = 1664525 * (z ^ 0xDEADBEEF) + 1013904223;
        return $$2 ^ $$3;
    }
    ...
    public static Stream<ChunkPos> stream(final ChunkPos pos1, final ChunkPos pos2) {
        int $$2 = Math.abs(pos1.x - pos2.x) + 1;
        int $$3 = Math.abs(pos1.z - pos2.z) + 1;
        final int $$4 = pos1.x < pos2.x ? 1 : -1;
        final int $$5 = pos1.z < pos2.z ? 1 : -1;
        return StreamSupport.stream(new Spliterators.AbstractSpliterator<ChunkPos>((long)($$2 * $$3), 64){
            ...
            public boolean tryAdvance(Consumer<? super ChunkPos> consumer) {
                if (this.position == null) {
                    ...
                    int $$1 = this.position.x;
                    int $$2 = this.position.z;
                    if ($$1 == pos2.x) {
                        if ($$2 == pos2.z) {
                            return false;
                        }
                        this.position = new ChunkPos(pos1.x, $$2 + $$5);
                    } else {
                        this.position = new ChunkPos($$1 + $$4, $$2);
                    }
                }
