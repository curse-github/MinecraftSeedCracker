   ...
   implements BiomeResolver
 {
   ...
   private final Supplier<Set<Holder<Biome>>> possibleBiomes = Suppliers.memoize(() -> (Set)collectPossibleBiomes().distinct().collect(ImmutableSet.toImmutableSet()));
   public Set<Holder<Biome>> possibleBiomes() { return (Set)this.possibleBiomes.get(); }
     ...
     for (int row = 0; row < h; row++) {
       for (int column = 0; column < w; column++) {
         for (int depth = 0; depth < d; depth++) {
           ...
           int noiseY = y0 + depth;
           int noiseZ = z0 + row;
           biomeSet.add(getNoiseBiome(noiseX, noiseY, noiseZ, sampler));
         } 
       } 
     } 
     return biomeSet;
   }
   public Pair<BlockPos, Holder<Biome>> findBiomeHorizontal(int x, int y, int z, int searchRadius, Predicate<Holder<Biome>> allowed, RandomSource random, Climate.Sampler sampler) {
                return findBiomeHorizontal(x, y, z, searchRadius, 1, allowed, random, false, sampler);
            }
   public Pair<BlockPos, Holder<Biome>> findClosestBiome3d(BlockPos origin, int searchRadius, int sampleResolutionHorizontal, int sampleResolutionVertical, Predicate<Holder<Biome>> allowed, Climate.Sampler sampler, LevelReader level) {
     Set<Holder<Biome>> candidateBiomes = (Set)possibleBiomes().stream().filter(allowed).collect(Collectors.toUnmodifiableSet());
     ...
     for (BlockPos.MutableBlockPos sampleColumn : BlockPos.spiralAround(BlockPos.ZERO, sampleRadius, Direction.EAST, Direction.SOUTH)) {
       ...
       for (int blockY : sampleYs) {
         ...
         Holder<Biome> biome = getNoiseBiome(noiseX, noiseY, noiseZ, sampler);
   ...
   public Pair<BlockPos, Holder<Biome>> findBiomeHorizontal(int originX, int originY, int originZ, int searchRadius, int skipSteps, Predicate<Holder<Biome>> allowed, RandomSource random, boolean findClosest, Climate.Sampler sampler) {
     ...
     for (currentRadius = startRadius; currentRadius <= noiseRadius; currentRadius += skipSteps) {
       int z; for (z = (SharedConstants.DEBUG_ONLY_GENERATE_HALF_THE_WORLD || SharedConstants.debugGenerateSquareTerrainWithoutNoise) ? 0 : -currentRadius; z <= currentRadius; z += skipSteps) {
         boolean zEdge = (Math.abs(z) == currentRadius); int x;
         for (x = -currentRadius; x <= currentRadius; x += skipSteps) {
           if (findClosest) {
