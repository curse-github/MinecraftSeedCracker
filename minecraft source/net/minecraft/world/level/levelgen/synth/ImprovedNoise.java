package net.minecraft.world.level.levelgen.synth;
 ...
 public final class ImprovedNoise
 {
    ...
    public double noise(double _x, double _y, double _z, double yScale, double yFudge) {
        ...
        if (yScale != 0.0D) {
            double fudgeLimit;
            if (yFudge >= 0.0D && yFudge < yr) {
                fudgeLimit = yFudge;
            } else {
                fudgeLimit = yr;
            }
            yrFudge = Mth.floor(fudgeLimit / yScale + 1.0000000116860974E-7D) * yScale;
        } else {
            yrFudge = 0.0D;
        }
    ...
    private double sampleAndLerp(int x, int y, int z, double xr, double yr, double zr, double yrOriginal) {
        ...
        double d100 = gradDot(p(xy10 + z), xr - 1.0D, yr, zr);
        double d010 = gradDot(p(xy01 + z), xr, yr - 1.0D, zr);
        double d110 = gradDot(p(xy11 + z), xr - 1.0D, yr - 1.0D, zr);
        double d001 = gradDot(p(xy00 + z + 1), xr, yr, zr - 1.0D);
        double d101 = gradDot(p(xy10 + z + 1), xr - 1.0D, yr, zr - 1.0D);
        double d011 = gradDot(p(xy01 + z + 1), xr, yr - 1.0D, zr - 1.0D);
        double d111 = gradDot(p(xy11 + z + 1), xr - 1.0D, yr - 1.0D, zr - 1.0D);
    ...
    private double sampleWithDerivative(int x, int y, int z, double xr, double yr, double zr,
        ...
        double d100 = SimplexNoise.dot(g100, xr - 1.0D, yr, zr);
        double d010 = SimplexNoise.dot(g010, xr, yr - 1.0D, zr);
        double d110 = SimplexNoise.dot(g110, xr - 1.0D, yr - 1.0D, zr);
        double d001 = SimplexNoise.dot(g001, xr, yr, zr - 1.0D);
        double d101 = SimplexNoise.dot(g101, xr - 1.0D, yr, zr - 1.0D);
        double d011 = SimplexNoise.dot(g011, xr, yr - 1.0D, zr - 1.0D);
        double d111 = SimplexNoise.dot(g111, xr - 1.0D, yr - 1.0D, zr - 1.0D);
        double xAlpha = Mth.smoothstep(xr);
        double yAlpha = Mth.smoothstep(yr);
        double zAlpha = Mth.smoothstep(zr);
        double d1x = Mth.lerp3(xAlpha, yAlpha, zAlpha, g000[0], g100[0], g010[0], g110[0], g001[0], g101[0],
                g011[0], g111[0]);
        double d1y = Mth.lerp3(xAlpha, yAlpha, zAlpha, g000[1], g100[1], g010[1], g110[1], g001[1], g101[1],
                g011[1], g111[1]);
        double d1z = Mth.lerp3(xAlpha, yAlpha, zAlpha, g000[2], g100[2], g010[2], g110[2], g001[2], g101[2],
                g011[2], g111[2]);
