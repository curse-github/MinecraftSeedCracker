package net.minecraft.world.level.levelgen.synth;
 ...
 public class NormalNoise
 {
    ...
    private static double expectedDeviation(int octaveSpan) {
        return 0.1D * (1.0D + 1.0D / (octaveSpan + 1));
    }
    public double getValue(double x, double y, double z) {
        double x2 = x * 1.0181268882175227D;
        double y2 = y * 1.0181268882175227D;
        double z2 = z * 1.0181268882175227D;
    ...
    public static final class NoiseParameters extends Record {
        ...
        public static final Codec<NoiseParameters> DIRECT_CODEC = RecordCodecBuilder
                .create(i -> i
                        .group(Codec.INT/* 117 */ .fieldOf("firstOctave").forGetter(NoiseParameters::firstOctave),
                                Codec.DOUBLE/* 118 */ .listOf().fieldOf("amplitudes")
                                        .forGetter(NoiseParameters::amplitudes))
                        .apply(i, NoiseParameters::new));
        public static final Codec<Holder<NoiseParameters>> CODEC = RegistryFileCodec.create(Registries.NOISE,
                DIRECT_CODEC);
        ...
        public NoiseParameters(int firstOctave, double firstAmplitude, double... amplitudes) {
            this(firstOctave,
                    (DoubleList) Util.make(new DoubleArrayList(amplitudes), list -> list.add(0, firstAmplitude)));
        }
    }
    }
